...
etc ...
```
Run your local database (Postgres)
...
<p>
  ...
  Divjoy docs for setting up Auth0: https://docs.divjoy.com/auth0
```js
...
# Setting up a new database locally

0. Install Prisma `npm install prisma --save-dev` and run `npx prisma init`
1. [Install postgres](https://www.postgresql.org/download)
2. Create a super user named "postgres", save the password offline
3. Log in as super user and create the database

```
~$ psql -U postgres
=# CREATE DATABASE product_roast;
```

Press Ctrl+d to exit the database interface.
...
-----------------+----------+----------+---------+-------+-----------------------
 product_roast   | postgres | UTF8     | C       | C     |
```

4. Create a new postgres user role with minimal permissions using the
   interactive user role creator tool.

```
~$ createuser --interactive --pwprompt -U postgres
Enter name of role to add: product_roast
...
---------------+------------------------------------------------+-----------
 product_roast | Create DB                                      | {}
 postgres      | Superuser, Create role, Create DB, Replication | {}
```

Press Ctrl+d to exit the database interface.

5. Update `.env` with Postgres connection string

`DATABASE_URL="postgresql://product_roast:randompassword@localhost:5432/product_roast?schema=public"`
...
You should now have have access to the Prisma admin panel as well which you can access by running:

`npx prisma studio`

And then navigating to `localhost:5555`

7. Create a migration anytime there is a change to the database schema
